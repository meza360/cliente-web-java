...
<section class="">
    ...
    <table class="table table-striped table-light container">
        ...
        <%
            int metodo = Integer.parseInt(request.getAttribute("metodo").toString());
            System.out.println("El metodo en el jsp de busqueda es: " + metodo);
            
           switch (metodo) {
                   case 1:
                       int codigo = Integer.parseInt(request.getAttribute("Codigo").toString());
                       System.out.println("El codigo a buscar en el jsp es: " + codigo);
                       Medicamento medicamento = new MedicamentoCliente().listarcodigo(codigo);
                       
        %>
                       
        <tr>
            <td><%= medicamento.getCodigo()%></td>
            <td><%= medicamento.getMedicamento()%></td>
            <td><%= medicamento.getDosis()%></td>
            <td><%= medicamento.getPrecioUnitario()%></td>
            <td><%= medicamento.getCantidadExistencia()%></td>
            <td><%= medicamento.getLaboratorioFarmaceutico()%></td>
            <td><%= medicamento.getVencimiento()%></td>
            <td><%= medicamento.getPresentacion()%></td>
            <td> <img src="images/med.png" alt="fotografia_medicamento" width="50" height="50"> </td>
            <td>
                <a class="btn btn-light btn-outline-dark" href="ManejoPeticiones.do?accion=listarcodigo&Codigo=<%= medicamento.getCodigo()%>">Detalles</a>
            </td>
        </tr>
        <%
                       break;
                   case 2:
                       String nombre = request.getAttribute("Medicamento").toString();
                       System.out.println("El nombre de medicamento a buscar en jsp es: " + nombre);
                       List<Medicamento> lista = new MedicamentoCliente().listarNombre(nombre);
        ...
        <%
                       ...
                       break;
                   case 3:
                       String laboratorio = request.getAttribute("Laboratorio").toString();
                       System.out.println("El laboratorio a buscar en jsp es: " + laboratorio);
                       List<Medicamento> lista2 = new MedicamentoCliente().listarLaboratorio(laboratorio);
                       for(Medicamento u:lista2){
                       
        %>
        <tr>
            <td><%= u.getCodigo()%></td>
            <td><%= u.getMedicamento()%></td>
            <td><%= u.getDosis()%></td>
            <td><%= u.getPrecioUnitario()%></td>
            <td><%= u.getCantidadExistencia()%></td>
            <td><%= u.getLaboratorioFarmaceutico()%></td>
            <td><%= u.getVencimiento()%></td>
            <td><%= u.getPresentacion()%></td>
            <td> <img src="images/med.png" alt="fotografia_medicamento" width="50" height="50"> </td>
            <td>
                <a class="btn btn-light btn-outline-dark" href="ManejoPeticiones.do?accion=listarcodigo&Codigo=<%= u.getCodigo()%>">Detalles</a>
            </td>
        </tr>
        <%
            }
